...
use await_expression::TraverseAwaitExpression;
use oxc::ast::{ast::Expression, match_member_expression};

#[allow(unused_variables)]
pub trait TraverseExpression<'a>: EntityHost<'a> + TraverseAwaitExpression<'a> {
...
impl<'a, H: Host<'a>> Analyzer<'a, H> {
  pub fn exec_expression(&mut self, node: &'a Expression<'a>) -> H::Entity {
    self.host.before_expression(node);
    ...
    self.host.after_expression(node, value)
  }
}
